...
3. Low-Resource Anchoring: Scan the opaque Ayoreo chunks for shared entities (e.g., proper nouns, numbers, punctuation patterns like quotation marks, paragraph size heuristics) that transcend translation to anchor to the ES/EN maps.
...
def main():
    ...
    print(f"Found {len(mismatches)} mismatched stories. Testing one...")
    
    # Take just one for experimental alignment evaluation
    test_id = next(iter(mismatches))
    
    alignment_map = align_story(client, test_id, mismatches[test_id])
    ...
    if alignment_map:
        ...
        dataset[test_id]["alignment_map"] = json.dumps(alignment_map)
        with open(JSON_PATH, "w", encoding="utf-8") as f:
            json.dump(dataset, f, indent=2, ensure_ascii=False)
        print("Saved experimental update back to ayoreoorg.json!")
